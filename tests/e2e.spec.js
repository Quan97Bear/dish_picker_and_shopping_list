...
test('select, complete, reopen the URL and create a shopping list', async ({ page }) => {
  ...
  const shoppingTab = page.getByRole('tab',{name:'采购清单'});
  const recipesTab = page.getByRole('tab',{name:'菜谱'});
  await expect(shoppingTab).toHaveAttribute('aria-selected','true');
  await expect(recipesTab).toHaveAttribute('aria-selected','false');
  ...
  await recipesTab.press('ArrowLeft');
  await expect(shoppingTab).toBeFocused();
  await expect(shoppingTab).toHaveAttribute('aria-selected','true');
  await expect(page.getByRole('heading',{name:'采购清单'})).toBeVisible();
  await expect(firstShoppingItem).toBeChecked();
  const shoppingCollapse = page.getByRole('button',{name:'收起整个采购清单'});
  await shoppingCollapse.click();
  const shoppingExpand = page.getByRole('button',{name:'展开整个采购清单'});
  await expect(shoppingExpand).toHaveAttribute('aria-expanded','false');
  await expect(page.getByRole('heading',{name:'采购清单'})).toBeVisible();
  await expect(firstShoppingItem).toBeHidden();
  await shoppingExpand.click();
  await expect(page.getByRole('button',{name:'收起整个采购清单'})).toHaveAttribute('aria-expanded','true');
  await expect(firstShoppingItem).toBeChecked();
});
