...
本地候选版本使用 `index-sXlXMcCz.js` + `index-DXeBjzVA.css`；生产仍使用 `index-DPcHz6I8.js` + `index-DXeBjzVA.css`。

## 当前产品差异

1. 本地候选版已将个性化称呼改为通用文案，尚未发布到生产。
2. 结果页尚未提供次级分享入口；系统分享、复制链接和二维码仍待接入新流程。
3. “推荐新菜 / 想新增这些菜”因选菜流程改变而延后重新设计。
4. 日历导出尚未实现。
5. iPhone Safari、微信和横屏视觉回归已完成；较大字体真机检查移至 P3 / P4。
...
- 360px 浏览器复核：选菜首页、新菜建议、建议分享弹层和建议接收页均无横向滚动或个性化称呼
- Vite 构建、部署产物和文档检查通过
- 本地资源：`index-sXlXMcCz.js` 与 `index-DXeBjzVA.css`
