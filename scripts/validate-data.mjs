...
import { validateIngredientGuide } from '../src/domain/ingredient-guide.js';
const read = (name) => JSON.parse(fs.readFileSync(new URL(`../data/${name}`, import.meta.url), 'utf8'));
const full = read('dishes.json'); const index = read('dish-index.json'); const schema = read('dishes.schema.json');
const ingredientGuide = read('ingredient-guide.json'); const ingredientGuideSchema = read('ingredient-guide.schema.json');
const ajv = new Ajv({ strict: false });
if (!ajv.validate(schema, full)) throw new Error(JSON.stringify(ajv.errors, null, 2));
if (!ajv.validate(ingredientGuideSchema, ingredientGuide)) throw new Error(JSON.stringify(ajv.errors, null, 2));
...
console.log(`数据校验通过：40 道菜，5 道汤，${ingredientGuide.ingredients.length} 种食材，ID、索引与食材指南一致。`);
