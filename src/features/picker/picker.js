...
const PANTRY_INGREDIENTS_KEY = 'home-menu-pantry-ingredients';
...
export function renderPicker(root, { index, dishes = [], ingredientGuide, initialSelected = [], initialServings = 2, initialNotes = {}, initialSuggestion = '', makeUrl, onComplete }) {
  ...
  function openPantry() {
    ...
    const backdrop = createPantryDialog({
      ...
      avoid,
      selectedKeys: pantryIngredients,
      menuSelectedIds: selected,
      onSelectionChange: (keys) => {
        pantryIngredients = new Set(keys);
        localStorage.setItem(PANTRY_INGREDIENTS_KEY, JSON.stringify([...pantryIngredients]));
      },
      onAddDish: (id) => {
        if (selected.includes(id)) return true;
        if (selected.length >= MAX_DISHES) {
          showToast('最多可选 12 道菜');
          return false;
        }
        toggle(id);
